...
from langgraph.checkpoint.memory import InMemorySaver  # Import the in-memory checkpointer
...
# 2. Setup the Tavily Search Tool
...
# 5. Create the agent and associate the checkpointer
agent = create_agent(
    model=llm,
    tools=tools,
    system_prompt=default_prompt,
    checkpointer=memory  # Enables conversational history tracking
)
...
def run_agent(question: str, config: dict):
    ...
    # We pass the config to agent.stream so it knows which conversation thread to load/save
...
if __name__ == "__main__":
    # Create a configuration mapping that links subsequent calls to the same thread ID
    config = {"configurable": {"thread_id": "user_session_1"}}
    
    print("--- Running Multi-Turn Conversation (Max 3 turns) ---")
    print("Type your message and press Enter. The agent will recall context from prior turns.")
